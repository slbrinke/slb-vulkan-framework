...
struct SpeciesUniforms {
    ...
    uint32_t maturityOrder; /**> Module order at which parameters switch to mature */
    float initialLambda;
    float matureLambda;
...
struct Prototype {
    ...
    float initialBudWeights[32]; /**< Initial lambda values of the terminal nodes accumulated through the prototype hierarchy */
    float matureBudWeights[32]; /**< Mature lambda values of the terminal nodes accumulated through the prototype hierarchy */
};
...
public:
    ...
    /**
     * Return the maximum module order.
     * 
     * @return maximum depth of the plant hierarchy in number of modules
     */
    ...
    /**
     * Change the branching angles for a given child branch segment.
     * 
     * The two angles be interpreted as polar coordinates of a child segment in the local coordinates of the parent.
     * 
     * @param childIndex index of the branch segment within the child list of the parent
     * @param theta vertical angle between parent and child branch in degrees
     * @param phi horizontal rotation around the parent branch in degrees
     */
    void setBranchingAngles(uint32_t childIndex, float theta, float phi);

    /**
     * Change the point at which a plant reaches maturity.
     * 
     * To keep it simple all modules beyond the specified order are considered mature.
     * 
     * @param order minimum order for a module to be considered mature
     */
    void setMaturityOrder(uint32_t order);

    /**
     * Change the resource distribution parameter used before a plant reaches maturity.
     * 
     * @param lambda amount of growth directed along the main axis relative to branching axes
     */
    void setInitialLambda(float lambda);

    /**
     * Change the resource distribution parameter used after a plant reaches maturity.
     * 
     * @param lambda amount of growth directed along the main axis relative to branching axes
     */
    void setMatureLambda(float lambda);

    /**
     * Change the number of iterations for module orientation optimization.
     * 
     * Effectively the maximum number of times the rotation of a module can change per optimization.
     * Each time a module is extended by a new order of branch segments the optimization is reset.
     * 
     * @param numIterations number of rotation optimization iterations
     */
    void setNumOptimizationIterations(uint32_t numIterations);
...
private:
    ...
    float m_maxModuleAge = 3.0f; /**< Maximum age at which a module stops growing */
    uint32_t m_maturityOrder = 2; /**< Minimum order for a module to be considered mature */
    ...
    uint32_t m_maxChildNodes = 2; /**< Maximum number of child branches sprouting out from a branch segment */
    ...
    float m_branchSizeDecrease[5] = {1.0f, 1.0f, 0.0f, 0.0f, 0.0f}; /**< Length of each child branch segment in relation to the parent branch segment length */
    ...
    float m_branchingPhis[5] = {glm::radians(30.0f), glm::radians(210.0f), 0.0f, 0.0f, 0.0f}; /**< Horizontal branching angle of each child branch segment relative to the orientation of the parent */
    float m_gravitropism = 0.01f; /**< Strength of the impact of gravity on branch segments */

    //simulation parameters
    float m_growthSpeed = 0.3f; /**< Maximum growth rate per simulated second */
    float m_initialLambda = 1.0f; /**< Resource distribution parameter used for the first few modules */
    float m_matureLambda = 0.5f; /**< Resource distribution paramter used once a plant matures */
