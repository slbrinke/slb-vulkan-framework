...
uint32_t PlantSpecies::getMaxNodesPerPrototype() {
    uint32_t base = 1;
    uint32_t sum = 0;
    for(uint32_t l=0; l<m_maxOrder; l++) {
        sum += base;
        base *= m_maxChildren;
    }
    return sum;
...
uint32_t PlantSpecies::createPrototypes(std::vector<Prototype> &prototypes, uint32_t &numNodes, std::vector<Node> &nodes) {
    ...
    for(uint32_t p=0; p<m_numPrototypes; p++) {
        ...
        uint32_t maxOffset = 1 + static_cast<uint32_t>((1.0f - lambda) * static_cast<float>(glm::pow(m_maxChildren, m_maxOrder-1) - 1));
        ...
        for(uint32_t l=0; l<m_maxOrder; l++) {
            ...
            for(uint32_t o=0; o<offset; o++) {
                ...
                nodes[nodeOffset+o].age = m_maxAge * (1.0f - (static_cast<float>(l) / static_cast<float>(m_maxOrder)));
